...
const CartLineItem = ({item, dispatch,REDUCER_ACTIONS}:PropsType) => {
    ...
    const optionsValue:number[]=[...Array(highestQuantity).keys()].map(i=>i+1)
    const options:ReactElement[] = optionsValue.map(val=>{
    ...
    const content = (
        <li className="cart__item">
            ...
            <div aria-label="Price Per Item">{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(item.price)}</div>
             ...
             <select name="itemQty"  className="cart__select" id="itemQty" value={item.qty}
             aria-label="Item Quantity" onChange={onChangeQty}>
             {options}
             </select>
