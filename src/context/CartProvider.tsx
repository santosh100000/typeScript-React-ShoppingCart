...
const reducer = (state:CartStateType, action:ReducerAction):CartStateType=>{
    switch(action.type){
        ...
        case REDUCER_ACTION_TYPE.REMOVE:{
            ...
            return{...action, cart:[...filteredCart]}
        }
        case REDUCER_ACTION_TYPE.QUANTITY:{
            if(!action.payload){  throw new Error(`action.payload is missing in Qty Action`)}
            const {sku,qty} = action.payload
                ...
                if(!itemExists){
                    throw new Error(`Items must exist to update an quantity`)
                }
